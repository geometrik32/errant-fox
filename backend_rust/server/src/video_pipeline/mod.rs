                    ...
                    Ok(res) => match &res {
                        ...
                        TranscodeSuccess { video_dst, logs } =>
                        {
                            ...
                            let linked_ok = (move || {
                                ...
                                #[cfg(windows)]
                                {
                                    if let Err(e) = std::os::windows::fs::symlink_file(&dst_filename, &symlink_path) {
                                        // Fallback to hard link on Windows
                                        if let Err(e2) = std::fs::hard_link(vh_dir.join(dst_filename), &symlink_path) {
                                            tracing::error!(details=%e2, "Failed to create symlink/hardlink {:?} -> {:?}", symlink_path, video_dst);
                                            return false;
                                        }
                                    }
                                }
