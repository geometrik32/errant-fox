...
fn create_sanitized_symlink(src_path: &PathBuf) -> Result<PathBuf, String> {
    ...
    #[cfg(windows)]
    if let Err(e) = std::os::windows::fs::symlink_file(src_filename, &sanitized_path) {
        // On Windows, symlinks often require admin privileges. Fallback to hard link or just use original?
        // For now, try hard link if symlink fails.
        if let Err(e2) = std::fs::hard_link(src_path, &sanitized_path) {
             return Err(format!("Failed to create sanitized symlink/hardlink on Windows: {} / {}", e, e2));
        }
    }
...
fn run_transcode_script(src: &CmprInputSource, output_dir: PathBuf, output_prefix: String,
    ...
    let ppipe_fname = match progress_pipe_path.to_str() {
        ...
        Some(fname) => {
            #[cfg(unix)]
            {
                unix_named_pipe::create(&fname, None)
            ...
            #[cfg(windows)]
            {
                tracing::warn!("Named pipes for progress tracking are not supported on Windows yet.");
                None
            }
        }
    };
