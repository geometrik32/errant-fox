...
use crate::{api_server::server_state::ServerState, grpc::{caller::OrganizerCaller, grpc_client::OrganizerURI}, auth::AuthManager};
...
impl ClapshotInit {
        ...
        -> anyhow::Result<Self>
    {
        ...
        // Ensure fighters and user accounts are synced on startup
        if let Ok(mut conn) = db.conn() {
            let _ = crate::database::models::HemaParticipant::ensure_users_synced(&mut conn);
        }
