                        ...
                        if msg.is_text() {
                            ...
                            let (cmd_str, json) = match parse_msg(&msg) {
                                Ok((cmd, json, cookies)) => {
                                    ...
                                    // Re-authenticate if a valid token is present in cookies
                                    if let Some(token) = ses.org_session.cookies.get("clapshot_token") {
                                        if let Ok(claims) = server.auth_manager.validate_token(token) {
                                            if ses.user_id != claims.sub {
                                                tracing::info!("Session {} re-authenticated as user '{}' via token in command.", sid, claims.sub);
                                                ses.user_id = claims.sub.clone();
                                                ses.user_name = claims.sub.clone(); // Use ID as name if not found in DB yet
                                                ses.is_admin = claims.is_admin;
                                                
                                                // Sync with org_session
                                                ses.org_session.is_admin = claims.is_admin;
                                                ses.org_session.user = Some(proto::UserInfo {
                                                    id: claims.sub,
                                                    name: ses.user_name.clone(),
                                                });

                                                // Update user info from DB if possible
                                                if let Ok(mut conn) = server.db.conn() {
                                                    if let Ok(user) = models::User::get(&mut conn, &ses.user_id) {
                                                        ses.user_name = user.name.clone();
                                                        if let Some(u) = ses.org_session.user.as_mut() {
...
/// * `auth_manager` - Auth manager for JWT validation
///
...
fn parse_auth_headers(hdrs: &HeaderMap, default_user_id: &str, filter_regex: &Regex, auth_manager: &crate::auth::AuthManager) -> (String, String, bool, HashMap<String, String>, HashMap<String, String>, Option<String>)
{
    ...
    // Check for native JWT auth in cookies
    let mut user_id = user_id;
    let mut user_name = user_name;
    let mut is_admin = is_admin;

    if let Some(token) = app_cookies.get("clapshot_token") {
        if let Ok(claims) = auth_manager.validate_token(token) {
            tracing::debug!("Valid JWT token found in cookies. Overriding header auth.");
            user_id = claims.sub;
            user_name = user_id.clone();
            is_admin = claims.is_admin;
        } else {
            tracing::warn!("Invalid JWT token found in cookies.");
        }
    }
    ...
    port: u16)
{
    ...
    let rt_api_ws = warp::path("api").and(warp::path("ws"))
        ...
        .map (move|hdrs: HeaderMap, ws: warp::ws::Ws| {
            ...
            // Get user ID and username (from reverse proxy or native JWT)
            let (user_id, user_name, is_admin, app_cookies, filtered_headers, remote_error) = parse_auth_headers(&hdrs, &server_state.default_user, &server_state.org_http_headers_regex, &server_state.auth_manager);
