...
{
    async fn db_get_hema_moves(&self, req: Request<org::DbGetHemaMovesRequest>) -> RpcResult<org::DbHemaMoveList>
    {
        use org::db_get_hema_moves_request::Filter;
        let req = req.into_inner();
        let db = self.server.db.clone();
        let pg = req.paging.as_ref().try_into()?;
        let conn = &mut db.conn()?;
        let items = match rpc_expect_field(&req.filter, "filter")? {
            Filter::All(_) => { models::HemaMove::get_all(conn, pg)? },
            Filter::Ids(ids) => {
                let ids = ids.ids.iter().map(|id| str_to_i32_or_tonic_error!(id)).collect::<Result<Vec<_>, _>>()?;
                paged_vec(models::HemaMove::get_many(conn, &ids)?, pg)
            },
        };
        Ok(Response::new(org::DbHemaMoveList {
            items: items.into_iter().map(|p: models::HemaMove| p.to_proto3()).collect(),
            paging: req.paging,
        }))
    }

    async fn db_get_hema_bouts(&self, req: Request<org::DbGetHemaBoutsRequest>) -> RpcResult<org::DbHemaBoutList>
    {
        use org::db_get_hema_bouts_request::Filter;
        let req = req.into_inner();
        let db = self.server.db.clone();
        let pg = req.paging.as_ref().try_into()?;
        let conn = &mut db.conn()?;
        let items = match rpc_expect_field(&req.filter, "filter")? {
            Filter::All(_) => { models::HemaBout::get_all(conn, pg)? },
            Filter::MediaFileId(vid) => { models::HemaBout::get_by_media_file(conn, &vid)? },
            Filter::ParticipantId(pid) => { models::HemaBout::get_by_participant(conn, *pid)? },
        };
        Ok(Response::new(org::DbHemaBoutList {
            items: items.into_iter().map(|p: models::HemaBout| p.to_proto3()).collect(),
            paging: req.paging,
        }))
    }
    ...
    async fn db_get_hema_participants(&self, req: Request<org::DbGetHemaParticipantsRequest>) -> RpcResult<org::DbHemaParticipantList>
    {
        use org::db_get_hema_participants_request::Filter;
        let req = req.into_inner();
        let db = self.server.db.clone();
        let pg = req.paging.as_ref().try_into()?;
        let conn = &mut db.conn()?;
        let items = match rpc_expect_field(&req.filter, "filter")? {
            Filter::All(_) => { models::HemaParticipant::get_all(conn, pg)? },
            Filter::Ids(ids) => {
                let ids = ids.ids.iter().map(|id| str_to_i32_or_tonic_error!(id)).collect::<Result<Vec<_>, _>>()?;
                paged_vec(models::HemaParticipant::get_many(conn, &ids)?, pg)
            },
        };
        Ok(Response::new(org::DbHemaParticipantList {
            items: items.into_iter().map(|p| p.to_proto3()).collect(),
            paging: req.paging,
        }))
    }

    async fn db_get_hema_media_participants(&self, req: Request<org::DbGetHemaMediaParticipantsRequest>) -> RpcResult<org::DbHemaMediaParticipantList>
    {
        use org::db_get_hema_media_participants_request::Filter;
        let req = req.into_inner();
        let db = self.server.db.clone();
        let pg = req.paging.as_ref().try_into()?;
        let conn = &mut db.conn()?;
        let items = match rpc_expect_field(&req.filter, "filter")? {
            Filter::All(_) => { models::HemaMediaParticipant::get_all(conn, pg)? },
            Filter::MediaFileId(vid) => { models::HemaMediaParticipant::get_by_media_file(conn, &vid)? },
            Filter::ParticipantId(pid) => { models::HemaMediaParticipant::get_by_participant(conn, *pid)? },
        };
        Ok(Response::new(org::DbHemaMediaParticipantList {
            items: items.into_iter().map(|p: models::HemaMediaParticipant| p.to_proto3()).collect(),
            paging: req.paging,
        }))
    }
    ...
    async fn db_upsert(&self, req: Request<org::DbUpsertRequest>) -> RpcResult<org::DbUpsertResponse>
    {
        ...
        Ok(Response::new(org::DbUpsertResponse {
            ...
            hema_participants: upsert_type!([
                conn, req.hema_participants, models::HemaParticipant, models::HemaParticipantInsert,
                |it: &proto::HemaParticipant| it.id == 0,
                |it: &models::HemaParticipant| Ok(it.to_proto3())])?,
            hema_bouts: upsert_type!([
                conn, req.hema_bouts, models::HemaBout, models::HemaBoutInsert,
                |it: &proto::HemaBout| it.id == 0,
                |it: &models::HemaBout| Ok(it.to_proto3())])?,
            hema_media_participants: {
                let items: Vec<models::HemaMediaParticipant> = req.hema_media_participants.iter()
                    .map(|it| models::HemaMediaParticipant::from_proto3(it))
                    .collect::<Result<Vec<_>, _>>()?;
                models::HemaMediaParticipant::insert_many(conn, &items)?
                    .into_iter().map(|it| it.to_proto3()).collect()
            },
            hema_moves: upsert_type!([
                conn, req.hema_moves, models::HemaMove, models::HemaMoveInsert,
                |it: &proto::HemaMove| it.id == 0,
                |it: &models::HemaMove| Ok(it.to_proto3())])?,
        }))
    }
    ...
    async fn db_delete(&self, req: Request<org::DbDeleteRequest>) -> RpcResult<org::DbDeleteResponse>
    {
        ...
        Ok(Response::new(org::DbDeleteResponse {
            ...
            hema_participants_deleted: models::HemaParticipant::delete_many(conn, req.hema_participant_ids.as_slice())? as u32,
            hema_bouts_deleted: models::HemaBout::delete_many(conn, req.hema_bout_ids.as_slice())? as u32,
            hema_moves_deleted: models::HemaMove::delete_many(conn, req.hema_move_ids.as_slice())? as u32,
        }))
    }
}
...
pub async fn run_org_to_srv_grpc_server(bind: GrpcBindAddr, server: ServerState) -> anyhow::Result<()>
{
    ...
    let service = org::organizer_outbound_server::OrganizerOutboundServer::new(OrganizerOutboundImpl { server: server.clone() });
    let auth_service = org::hema_auth_service_server::HemaAuthServiceServer::new(crate::grpc::auth_service::HemaAuthServiceImpl::new(server));

    run_organizer_outbound_grpc_server(bind, service, auth_service, span, server_listening_flag, terminate_flag).await
}
