...
impl GrpcBindAddr {
    pub fn to_uri(&self) -> String {
        match self {
            ...
            #[cfg(windows)]
            GrpcBindAddr::Unix(_) => panic!("Unix sockets not supported on Windows"),
...
impl Debug for GrpcBindAddr {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            #[cfg(windows)]
            GrpcBindAddr::Unix(_) => write!(f, "unix://(unsupported)"),
...
pub async fn run_organizer_outbound_grpc_server<S1, S2>(
    bind: GrpcBindAddr,
    service: proto::org::organizer_outbound_server::OrganizerOutboundServer<S1>,
    auth_service: proto::org::hema_auth_service_server::HemaAuthServiceServer<S2>,
    span: tracing::Span,
    server_listening_flag: Arc<AtomicBool>,
    terminate_flag: Arc<AtomicBool>) -> anyhow::Result<()>
where
    S1: proto::org::organizer_outbound_server::OrganizerOutbound,
    S2: proto::org::hema_auth_service_server::HemaAuthService,
{
    ...
    match bind {
        ...
        #[cfg(windows)]
        GrpcBindAddr::Unix(_) => {
             anyhow::bail!("Unix sockets not supported on Windows");
        }
        ...
        -> Result<OrganizerOutboundConnection, tonic::Status>
{
    ...
    let channel = match ep {
        ...
        #[cfg(windows)]
        grpc_endpoint::Endpoint::Unix(_) => {
             return Err(Status::unimplemented("Unix sockets not supported on Windows"));
        }
