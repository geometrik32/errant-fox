...
**Дробить задачи.** Один промпт = одна задача. Не писать "сделай весь плеер" — писать "сделай VideoPlayer без панелей".

**Всегда указывать файлы.** "Измени frontend/src/lib/player/VideoPlayer.svelte строки 45-80" дешевле чем "найди где-то и исправь".

**Использовать Plan mode перед сложным.** Получить план бесплатно, утвердить, потом Edit. Это экономит на переделках.

**Читать перед просьбой исправить.** Открыть файл в VS Code, выделить нужный кусок → Claude видит только его через ide_selection, не весь файл.

**Не просить объяснений.** Добавить в промпт: "Без объяснений, только код". Экономит ~30% токенов.

**Копировать из TrueNAS частями.** Переносить компонент → попросить убрать Clapshot-специфику → дешевле чем писать с нуля.
