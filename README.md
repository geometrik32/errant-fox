...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/AI-Faster--Whisper-green?style=for-the-badge&logo=python" alt="Faster-Whisper">
...
> ⚡ **Errant Fox** — это open-source веб-платформа для покадрового анализа видео спаррингов HEMA, интервальной разметки сходов (включая автоматическое ИИ-распознавание) и отслеживания фехтовальной статистики.

**Errant Fox** спроектирован как специализированный инструмент для фехтовальных клубов HEMA (Historical European Martial Arts, с базовым фокусом на длинный меч). Система позволяет бойцам и тренерам просматривать записи тренировок, размечать судейские сходы (ручной ввод или авторазметка через Faster-Whisper), классифицировать применяемые техники (например, удары из различных страж, обманные действия), отмечать зоны поражения на интерактивном силуэте фехтовальщика и отслеживать личную детальную статистику.

Проект разработан для развертывания на **домашнем сервере (TrueNAS SCALE)** с интеграцией облачного хранилища **Seafile** для видеофайлов. Система работает в режиме *single-tenant* (одна установка на клуб, все бойцы имеют общий доступ к анализируемым данным, с поддержкой генерации публичных гостевых ссылок на отдельные видео и сходы).
...
* **HEMA-секретари и организаторы** — для автоматизации разметки тренировок с помощью ИИ и сбора статистики использования техник.
...
* *«автоматическая разметка видео спаррингов whisper hrm hba»*
...
Проект использует легковесный и производительный стек технологий. Видеофайлы стримятся напрямую из Seafile в браузер клиента, минуя транзитный трафик через бэкенд. Автоматическая разметка отрезков боев производится микросервисом `whisper-service` (Python + Faster-Whisper, оптимизированный под CPU int8), а бэкенд на Rust генерирует покадровые превью с помощью FFmpeg и управляет базой данных SQLite.
...
flowchart LR
    ...
    subgraph Server ["Сервер TrueNAS SCALE (Docker)"]
        NGINX["Nginx Proxy"]
        BE["Бэкенд (Rust + Axum)"]
        WS_AI["Whisper Service (Python / Faster-Whisper)"]
        DB[("БД (SQLite)")]
        LocalData["Локальные превью, аватары и транскрипты"]
    end
    ...
    BE -->|Запросы ORM| DB
    BE -->|Запуск ИИ-анализа аудио| WS_AI
    WS_AI -->|Прямая загрузка аудио| Seafile
...
### 2. Автоматическая ИИ-разметка сходов (Whisper AI & Acoustic Refinement)
* **Распознавание судейских команд**: Микросервис на базе `Faster-Whisper` (`CTranslate2` int8 CPU) распознает ключевые стоп-слова («Стоп», «Бой», «Время» и др.) в аудиодорожке.
* **Акустическое уточнение пиков (Acoustic Peak Refinement)**: Алгоритм анализирует громкость звукового сигнала вокруг распознанного слова и точно привязывает границы схода к мгновенному пику судейского выкрика.
* **Пакетная разметка и отмена**: Удобная модалка пакетного ИИ-анализа для выделенных или всех неразмеченных видео с отображением прогресса в реальном времени по WebSocket и мгновенной отменой заданий.
* **Инспектор транскриптов**: Встроенный инструмент администратора для просмотра распознанного текста и точности попаданий пиков.

### 3. Профессиональный видеоплеер
...
* **Интерактивный таймлайн**: Отображает размеченные сходы (ручные и созданные ИИ) в виде цветных сегментов и точки комментариев. При клике на сход включается режим циклического воспроизведения (Loop) этого отрезка.

### 4. Панель разметки сходов (Bouts)
...
* **История изменений**: Ведется аудит действий над сходами с фиксацией отметок от ИИ и пользователей.

### 5. Встроенный чат и публичный шаринг
...
* **Публичные ссылки**: Возможность сгенерировать токен доступа к видео или конкретному сходу для внешних пользователей с поддержкой гостевых комментариев и скачивания.

### 6. Аналитический дашборд бойца
