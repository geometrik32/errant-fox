...
impl ServerState {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>) -> ServerState
    {
        ServerState {
            db,
    ...
    pub fn emit_cmd(&self, cmd: proto::client::server_to_client_cmd::Cmd, send_to: SendTo) -> Res<u32>
    {
        ...
        match send_to {
            ...
            SendTo::AllUsers => { self.send_to_all_sessions(&msg) },
        }
    }
    ...
    /// Send a message to all currently connected user sessions.
    /// Returns the total number of sessions notified.
    pub fn send_to_all_sessions(&self, msg: &warp::ws::Message) -> Res<u32> {
        let mut total_sent = 0u32;
        let map = self.sid_to_session.read();
