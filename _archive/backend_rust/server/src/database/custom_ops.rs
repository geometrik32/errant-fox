...
impl models::User {
    ...
    pub fn get_all(conn: &mut PooledConnection) -> DBResult<Vec<models::User>>
    {
        use schema::users::dsl::*;
        to_db_res(retry_if_db_locked!({
            users.order(created.desc()).load::<models::User>(conn)
        }))
    }

    pub fn delete(conn: &mut PooledConnection, uid: &str) -> EmptyDBResult
    {
        use schema::users::dsl::*;
        retry_if_db_locked!({
