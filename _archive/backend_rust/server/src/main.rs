...
fn main() -> anyhow::Result<()> {
    ...
    let auth_secret = std::env::var("CLAPSHOT_AUTH_SECRET").unwrap_or_else(|_| {
        let secret = "temporary_insecure_secret_change_me_in_production".to_string();
        tracing::warn!("CLAPSHOT_AUTH_SECRET env var not set. Using temporary secret. THIS IS INSECURE.");
        secret
    });
    let auth_manager = Arc::new(AuthManager::new(&auth_secret));
