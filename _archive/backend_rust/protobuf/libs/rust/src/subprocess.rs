                    ...
                    match event.token() {
                        Token(0) => {
                            match reader.read_line(&mut buffer) {
                                Ok(_size) => {
                                    let line = strip_ansi_escapes::strip_str(buffer.clone()).trim_end().to_string();
...
#[cfg(windows)]
pub fn spawn_shell(_cmd_str: &str, _name: &str, _span: tracing::Span) -> anyhow::Result<ProcHandle> {
    anyhow::bail!("Subprocess spawning not supported on Windows");
}
